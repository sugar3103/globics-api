...
exports.list = async (role, pageSize=1000000, page=1) => {
    ...
    let data = await query.fetchPage({
        pageSize: pageSize,
        page: page,
        withRelated: ['userInfo'],
        columns: RESPONSE_USER_COLUMNS
    });
    let total = data.pagination.rowCount;
...
exports.search = async (pageSize=1000000, page=1, search) => {
    ...
    let query = this.User.where({'deleted_at': null}).whereNotNull('activated_at');
...
exports.removeById = async (id) => {
    let user = await this.User.forge({id: id}).save({deleted_at: new Date()}, {patch: true});
