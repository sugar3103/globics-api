...
exports.validRefreshNeeded = (req, res, next) => {
  ...
  const hash = Utils.hash(req.jwt.userId + jwtSecret, req.jwt.refreshKey)
...
exports.validJWTNeeded = (req, res, next) => {
  ...
  try {
    ...
    if (!contentType || !contentType.includes('multipart')) {
      const requestLog = `${req.jwt.userId} ${req.method} ${req.originalUrl} ${req.jwt.email} ${req.headers['time-zone']} ${req.headers['user-agent']}`
