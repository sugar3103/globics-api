...
exports.minutesToTimeString = function (minutes) {
  const hour = (Math.floor(minutes / 60)).pad()
  const min = (minutes % 60).pad()
  return `${hour}:${min}:00`
}
...
exports.lastDateOfMonth = (month, year) => {
  const yearMonth = `${year}-${month.pad()}`
  const numberOfDays = moment(yearMonth, 'YYYY-MM').daysInMonth()
...
exports.convertDateStringToDateTimeWithTimeZone = (dateString, timeString = null, timeZoneId = constants.UTC_TIME_ZONE) => {
  const dateTimeString = timeString ? moment.tz(`${dateString} ${timeString}`, timeZoneId) : moment.tz(dateString, timeZoneId)
...
exports.newMomentTimezone = (dateString, timeString = constants.START_TIME_STRING, timezone) => {
  const dateTimeString = timeString ? moment.tz(`${dateString} ${timeString}`, timezone) : moment.tz(dateString, timezone)
